...
int main( int argc, char **argv )
{
   // Initialize program
	...
	if( programRunFlag == True && configUploadFlag == True )
	{
		...
		{
			//int fileNameLen = getStringLength(fileName);
			//bool isconfigdata = getConfigData(fileName, &configDataPtr,errorMessage);
			//printf("filename: %s length: %d facts: %d\n", fileName,fileNameLen,isconfigdata);
                        // show error message, end program
                          // function: printf
