...
int main(int argc, char** argv) {
	...
	while (programRunFlag == False && argIndex < argc) {
		...
		// function: getString Length, findSubString
	...
	if (programRunFlag == True && configUploadFlag == True) {
		...
		// function: getString Length, getConfigData
		if (getStringLength(fileName) > 0 && getConfigData(fileName, &configDataPtr, errorMessage)
			== True) {
		...
		// otherwise, assume config file upload failure
			...
			// show error message, end program
			// function: printf
			printf("\nConfig Uplpad Error: %s, program aborted\n\n ", errorMessage);
			// unset program run flag
			programRunFlag = False;
		}
	}
