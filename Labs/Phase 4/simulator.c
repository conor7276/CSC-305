...
pstcondition: Block the thread for some time
Exceptions: none
Notes: none
*/
...
Boolean mem_alloca_func(memory_t **mem_list, ConfigDataType * config, int base, int offset, int pid) {
	...
	while(mem){
		// go through the memory request list and check if this request overlaps with any existing request
...
Algorithm: master the process and execute the process oen by one
...
void runProcess(process_t* current_process, memory_t **mem_list, ConfigDataType *config) {
	...
	for(int i = 0; !mem_fault && i < current_process->exe_size; ++i){
		...
		switch(exec->command){
			...
			case DEVOUT:{
				sprintf(value, "Process: %d, %s output opertation start", current_process->pid, exec->origin->strArg1);
				output_with_time(value, config);
...
Exceptions: none
Notes: The process picked up will be removed from the process list
*/
...
conducts OS simulation with varyin scheduling strategies
and various number of processes

Precondition: given head pointer to config data and metadat
Postcondition: simulator is provided, file output is porvided as configured
Exceptions: none
Notes: none
*/
void runSim(ConfigDataType* configPtr, OpCodeType* metaDataMsterPtr) {
	...
	if(configPtr->logToCode == LOGTO_BOTH_CODE || configPtr->logToCode == LOGTO_FILE_CODE){
		...
		fprintf(fp,"Quantnum Cycles                 : %d\n", configPtr->quantumCycles);
		fprintf(fp,"MemoryAvailable(KB)             : %d\n", configPtr->memAvailable);
	...
	output_with_time("OS: Simulation Start", configPtr);
	...
	while (ptr != NULL) {
		...
		if (compareString(ptr->command, "sys") == 0) {
			...
			if (compareString(ptr->strArg1, "start") == 0) {
				...
				// set the range and the end pointer
				range++;
				app_end = ptr;
				// malloc the initial execution list object and initialize the value to zero
				...
				else{ // else it's the very first process
	...
	while((selected_proc = schedule(&process_list, configPtr)) != False){
		...
		// free the memory since the process is EXITED
		memory_t *mem = mem_list;
		while(mem){
			mem_list = mem->next;
			free(mem);
