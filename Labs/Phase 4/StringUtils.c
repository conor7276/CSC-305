...
/*
 ...
 Function Iutput/Parameters: c-style source test string (char *)
    ...
    if( startIndex >= 0 && startIndex <= endIndex && endIndex < sourceStrLen )
       {
        ...
        // release memoryfor temporary string
           // function: free
        free( tempStr );
       }
...
Postcondition: Output is the monitor, file or both.
Exceptions: None
Notes: None
*/
void output(char *message, ConfigDataType *configPtr){
   ...
   // if both, then weite to file and to screen
...
Precondition: given pointer to config data
PostconditionL Output to the mointor, file or both.
Exceptions: none
Notes: none
*/
